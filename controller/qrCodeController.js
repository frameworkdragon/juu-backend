...
const createQRCode = async (req, res) => {
  try {
    ...
    const alreadyExistingData = await Tag.findOne({ tagID }).populate('qrCode')
    if (
      alreadyExistingData ||
      alreadyExistingData?.qrCode?.qrCodeID == qrCodeID
    ) {
      return res
        .status(400)
        .json({
          success: false,
          errorMessage: 'QRCode or TagID already exists in DB.',
          errorType: 'Internal Server Error',
        })
    }
